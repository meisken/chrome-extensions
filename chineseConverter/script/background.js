...
function quickConvertToZh(string){
    ...
    const findTargetWord = (value) => {
        if(!isMatchTheQuickFormat(value)){
            return value
        }else{
            const spacesLength = value.match(/\s/g) === null ? 0 : value.match(/\s/g).length;
            ...
            const target = qdict.find((val) => {
                return val[1][0] === radicals[0] && val[1][1] === characterId
            });
    ...
    for(let i = 0; i < separatedWordsArray.length; i++ ){
        const word = findTargetWord(separatedWordsArray[i]);
        words.push(word);
    };
